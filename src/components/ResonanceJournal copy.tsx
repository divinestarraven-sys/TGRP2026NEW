...
export default function ResonanceJournal() {
  ...
  return (
    <>
      <motion.button
        className="shrink-0 w-14 h-14 rounded-full glass-gold glow-amber flex items-center justify-center"
        ...
        {isOpen && (
          ...
          >
            ...
            <div className="p-3 border-b border-solarpunk-moss/15">
              ...
              <div className="flex gap-2">
                <textarea
                  ...
                  rows={2}
                  className="flex-1 bg-cosmic-deep/50 border border-gold-sacred/15 rounded-xl px-4 py-2.5 text-sm font-body text-moonlight-white placeholder:text-moonlight-white/80 focus:outline-none focus:border-gold-sacred/30 transition-colors resize-none"
                />
                <button
                  onClick={addEntry}
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-3 min-h-[120px] max-h-[280px]">
              {entries.length === 0 && (
                <p className="font-sacred text-moonlight-white/80 text-sm text-center py-6 italic">
              ...
              {entries.map((entry) => (
                ...
                >
                  <div className="flex items-center justify-between mb-1">
                    <span className="text-[15px] font-display tracking-wider text-gold-sacred/80">{entry.date}</span>
                    <div className="flex items-center gap-2">
                      <span className="text-[15px] font-display tracking-wider text-moonlight-white/80">{entry.pillar}</span>
                      <button
                        onClick={() => deleteEntry(entry.id)}
                        className="opacity-0 group-hover:opacity-100 transition-opacity"
                      >
                        <Trash2 className="w-3 h-3 text-moonlight-white/80 hover:text-red-400/60" />
                      </button>
                    </div>
                  </div>
                  <p className="font-body text-sm text-moonlight-white/80 leading-relaxed">{entry.text}</p>
                </motion.div>
              ))}
            </div>
          </motion.div>
