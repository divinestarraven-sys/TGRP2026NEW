          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-5xl mx-auto">
            {projectDownloads.length === 0 ? (
              <GlassCard gold className="p-6 sm:p-8 md:col-span-2 text-center">
                <FileText className="w-7 h-7 text-gold-sacred/80 mx-auto mb-4" aria-hidden="true" />
                <h3 className="font-display text-xl text-moonlight-white/80 mb-3">Coming Soon</h3>
                <p className="font-body text-moonlight-white/80 text-base leading-relaxed">
              ...
              projectDownloads.map((resource) => (
                <GlassCard key={resource.href} gold className="p-6 sm:p-8">
                  ...
                  <p className="font-body text-moonlight-white/80 text-base leading-relaxed mb-5">
                    ...
                    {category.items.map((item) => (
                      <motion.div key={item.title} whileHover={{ x: item.url ? 4 : 0 }}>
                        {item.url ? (
                          ...
                          >
                            <div>
                              <p className="font-body text-sm text-moonlight-white/80 group-hover:text-emerald-glow transition-colors">
                                {item.title}
                              </p>
                              <span className="text-xs text-moonlight-white/80 font-display tracking-wider">{item.type}</span>
                            </div>
                            <ExternalLink className="w-4 h-4 text-moonlight-white/80 group-hover:text-emerald-glow/40 transition-colors shrink-0" />
                          </a>
                        ) : (
                          <div className="flex items-center justify-between p-3 rounded-lg cursor-default">
                            <div>
                              <p className="font-body text-sm text-moonlight-white/80">
                                {item.title}
                              </p>
                              <span className="text-xs text-moonlight-white/80 font-display tracking-wider">{item.type} &middot; Coming Soon</span>
                            </div>
                          </div>
                        )}
                      </motion.div>
            ...
            {externalLinks.map((link, i) => (
              <GlassCard key={link.title} delay={i * 0.1} className="p-5 cursor-default">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h4 className="font-display text-sm tracking-wider text-moonlight-white/80">
                      {link.title}
                    </h4>
                    <p className="text-xs text-moonlight-white/80 font-body">{link.desc}</p>
                  </div>
                </div>
              </GlassCard>
            ))}
        ...
        <div className="container-sacred text-center">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 max-w-3xl mx-auto mb-10">
            <GlassCard solar className="p-6">
              ...
              <p className="font-body text-moonlight-white/80 text-xs mb-3">Free PDF introduction to the framework</p>
              <Link
                to="/codex"
            ...
            <GlassCard gold className="p-6">
              ...
              <p className="font-body text-moonlight-white/80 text-xs mb-3">Weekly reflections and updates</p>
              <Link
                to="/join"
            ...
            <GlassCard solar className="p-6">
              ...
              <p className="font-body text-moonlight-white/80 text-xs mb-3">Living archive of symbols & systems</p>
              <Link
                to="/codex"
